...
int compare(SORT_TYPE *a, SORT_TYPE *b)
{
    if (*a > *b)
        return 1;
    else if (*a < *b)
...
namespace grailsort {
    ...
    struct inplace_merger {
        ...
        static void GrailSortWithBuffer(SORT_TYPE *arr, int Len) {
            SORT_TYPE ExtBuf[GRAIL_EXT_BUFFER_LENGTH];
            grail_commonSort(arr, Len, ExtBuf, GRAIL_EXT_BUFFER_LENGTH);
        }
        static void GrailSortWithDynBuffer(SORT_TYPE *arr, int Len) {
            int L = 1;
            SORT_TYPE *ExtBuf;
            while (L*L < Len) L *= 2;
    ...
    void sort(iter begin, iter end, Compare less) {
        ...
        index n = end - begin;
        inplace_merger<t>::GrailSort(&(*begin), n);
    }
    ...
    void sort(std::vector<T> &v) {
        ...
        grailsort::sort(begin, end, std::less<T>());
