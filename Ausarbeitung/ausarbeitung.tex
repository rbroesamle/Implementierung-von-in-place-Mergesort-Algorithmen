...
\end{tabular}
\normalsize \\
In the above tests (except Fig.\ref{fig:ints-time-otherNormalization}), we run the in-place algorithm of Reinhardt by using insertion sort in \textit{"reinhardt\_gapsort"} for small cases and the asymmetric merge in \textit{"reinhardt\_merge"}. Furthermore we utilised the quickselect step in every iteration. \\ %%Continue here...
