...
\caption{ $ \frac{Time(ns)}{n*log(n)} $ for different Bigtype list sizes $ n $ } \label{fig:big-beides-Teuer}
\end{figure}
...
\end{figure}
In the test from Fig. \ref{fig:ints-time-otherNormalization} we did not use the usual insertion sort for small sublists as its usage leads to a varying number of additional comparisons depending on how the list is partitioned. Therefore, we need less comparisons compared to the usual configurations. \\ %% Continue here...
